...
const VideoNode = Node.create({
    ...
    renderHTML({ HTMLAttributes }) {
        const { controls, ...rest } = HTMLAttributes as any;
        // boolean 속성은 존재만 해도 true
        return ['video', { ...(controls ? { controls: '' } : {}), ...rest }];
    },
    ...
    return (
        <div className="border rounded bg-white">
            ...
            <div className="flex flex-wrap gap-1 px-2 py-2 border-b bg-neutral-50 text-sm">
                {/* 인라인 */}
                <button type="button" aria-pressed={activeBold}
                        onClick={() => editor.chain().focus().toggleBold().run()}
                        className={`${btn} ${activeBold ? btnOn : btnOff}`} disabled={!canBold}>B</button>

                <button type="button" aria-pressed={activeItalic}
                        onClick={() => editor.chain().focus().toggleItalic().run()}
                        className={`${btn} italic ${activeItalic ? btnOn : btnOff}`} disabled={!canItalic}>/</button>

                <button type="button" aria-pressed={activeStrike}
                        onClick={() => editor.chain().focus().toggleStrike().run()}
                        className={`${btn} ${activeStrike ? btnOn : btnOff}`} disabled={!canStrike}>S</button>

                <button type="button" aria-pressed={activeUnderline}
                        onClick={() => editor.chain().focus().toggleUnderline?.().run()}
                        className={`${btn} ${activeUnderline ? btnOn : btnOff}`}>U</button>

                {/* 목록 */}
                <button type="button" aria-pressed={activeBullet}
                        onClick={() => editor.chain().focus().toggleBulletList().run()}
                        className={`${btn} ${activeBullet ? btnOn : btnOff}`} disabled={!canBullet}>• List</button>

                <button type="button" aria-pressed={activeOrdered}
                        onClick={() => editor.chain().focus().toggleOrderedList().run()}
                        className={`${btn} ${activeOrdered ? btnOn : btnOff}`} disabled={!canOrdered}>1. List</button>

                {/* 인용 */}
                <button type="button" aria-pressed={activeQuote}
                        onClick={() => editor.chain().focus().toggleBlockquote().run()}
                        className={`${btn} ${activeQuote ? btnOn : btnOff}`} disabled={!canQuote}>&quot;</button>

                {/* 제목 */}
                <button type="button" aria-pressed={activeH2}
                        onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()}
                        className={`${btn} ${activeH2 ? btnOn : btnOff}`} disabled={!canH2}>H2</button>

                <button type="button" aria-pressed={activeH3}
                        onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()}
                        className={`${btn} ${activeH3 ? btnOn : btnOff}`} disabled={!canH3}>H3</button>

                {/* 되돌리기/다시 */}
                <button type="button"
                        onClick={() => editor.chain().focus().undo().run()}
                        className={`${btn} ${btnOff}`} disabled={!editor.can().undo()}>↶</button>

                <button type="button"
                        onClick={() => editor.chain().focus().redo().run()}
                        className={`${btn} ${btnOff}`} disabled={!editor.can().redo()}>↷</button>

                {/* Clear */}
                <button type="button"
                        onClick={() => editor.chain().focus().unsetAllMarks().clearNodes().run()}
                        className={`${btn} ${btnOff}`}>Clear</button>

                {/* 색상/크기 */}
                <input type="color"
                       onChange={e => editor.chain().focus().setColor(e.target.value).run()}
                       className="ml-2 h-8 w-12 border rounded" title="글자 색상" />

                <select className="ml-1 h-8 border rounded px-2" defaultValue=""
                        onChange={e => setFontSize(e.target.value)} title="글자 크기">
                ...
                {/* 링크 */}
                <button type="button" className={`${btn} ${btnOff}`} title="링크"
                        onClick={() => {
          ...
          <button type="button" className={`${btn} ${btnOff}`} onClick={() => setImgWidth('100%')}>100%</button>
          <button type="button" className={`${btn} ${btnOff}`} onClick={() => setImgWidth('75%')}>75%</button>
          <button type="button" className={`${btn} ${btnOff}`} onClick={() => setImgWidth('50%')}>50%</button>
          <button type="button" className={`${btn} ${btnOff}`} onClick={() => setImgWidth(undefined)}>원본</button>
        </span>
